...
def random_crop(img,den, oa_gt,dst_size):
    # dst_size: ht, wd
    ...
    return img[:,y1:y2,x1:x2], den[label_y1:label_y2,label_x1:label_x2], oa_gt[label_y1:label_y2,label_x1:label_x2]
...
def SHHA_collate(batch):
    ...
    imgs, dens,oa_gt = [transposed[0],transposed[1], transposed[2]]

    error_msg = "batch must contain tensors; found {}"
    if isinstance(imgs[0], torch.Tensor) and isinstance(dens[0], torch.Tensor) and isinstance(oa_gt[0], torch.Tensor):
        ...
        for i_sample in range(len(batch)):
            _img, _den, _oa_gt = random_crop(imgs[i_sample],dens[i_sample], oa_gt[i_sample],[min_ht,min_wd])
            cropped_imgs.append(_img)
            cropped_dens.append(_den)
            cropped_oa_gts.append(_oa_gt)
        ...
        cropped_oa_gts = torch.stack(cropped_oa_gts, 0, out=share_memory(cropped_oa_gts))

        return [cropped_imgs,cropped_dens, cropped_oa_gts,transposed[3]]
